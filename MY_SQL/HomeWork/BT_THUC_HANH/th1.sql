...
-- 9
DELIMITER //
CREATE TRIGGER CK_SL_SP 
BEFORE INSERT ON th1quanlybanhang.HOADON 
FOR EACH ROW
BEGIN
DECLARE SLSP INT;
SELECT COUNT(*) INTO SLSP 
FROM th1quanlybanhang.CTHD 
WHERE SOHD = NEW.SOHD;
	IF SOSP >= 1 THEN
		SIGNAL SQLSTATE '45000' 
		SET MESSAGE_TEXT = 'THANH CONG';
	ELSE
		SIGNAL SQLSTATE '45000' 
		SET MESSAGE_TEXT = ' KHONG THE CHEN VI MOI LAN MUA IT NHAT 1 SAN PHAM';
  END IF;
END //
...
DELIMITER //
CREATE TRIGGER CK_HD
BEFORE INSERT ON th1quanlybanhang.khachhang_copy
FOR EACH ROW
BEGIN
  DECLARE NGAYHD DATE;
  SELECT H.NGHD INTO NGAYHD
  FROM th1quanlybanhang.hoadon AS H
  WHERE H.MAKH = NEW.MAKH;
  IF NGAYHD >= NEW.NGDK THEN
	SIGNAL SQLSTATE '45000' 
    SET MESSAGE_TEXT = 'THANH CONG';
  ELSE
    SIGNAL SQLSTATE '45000' 
    SET MESSAGE_TEXT = 'KHONG THE CHEN KHACH HANG VI DA CO HOA DON SAU NGAY DANG KY';
  END IF;
END//
...
DELIMITER //
CREATE TRIGGER CK_HDNV 
BEFORE INSERT ON th1quanlybanhang.hoadon
FOR EACH ROW
BEGIN
  DECLARE NGAYVL DATE;
  SELECT V.NGVL INTO NGAYVL
  FROM th1quanlybanhang.NHANVIEN AS V
  WHERE V.MANV = NEW.MANV;
  IF NGAYVL <= NEW.NGHD THEN
	SIGNAL SQLSTATE '45000' 
    SET MESSAGE_TEXT = 'THANH CONG';
  ELSE
    SIGNAL SQLSTATE '45000' 
    SET MESSAGE_TEXT = 'KHONG THE CHEN VI NGAY BAN HANG PHAI LON HON NGAY VAO LAM CUA NHAN VIEN ';
  END IF;
END//
...
-- 9
DELIMITER //
CREATE TRIGGER CK_CTHD
BEFORE INSERT ON th1quanlybanhang.HOADON
FOR EACH ROW 
BEGIN
	DECLARE CTHDON VARCHAR(5);
    SELECT C.SOHD INTO CTHDON
    FROM th1quanlybanhang.CTHD AS C
    WHERE C.SOHD = NEW.SOHD;
	IF CTHDON >= NEW.SOHD THEN
        SIGNAL SQLSTATE '45000' 
		SET MESSAGE_TEXT = 'THANH CONG';
	ELSE
		SIGNAL SQLSTATE '45000' 
		SET MESSAGE_TEXT = 'KHONG THE CHEN VI 1 HOA DON PHAI CO NHIEU CTHD';
    END IF;
END //
...
-- 10
DELIMITER //
CREATE TRIGGER CK_TRIGIA
BEFORE INSERT ON th1quanlybanhang.HOADON
FOR EACH ROW 
BEGIN
	DECLARE CTHDON VARCHAR(5);
    SELECT C.SL INTO CTHDON
    FROM th1quanlybanhang.CTHD AS C,th1quanlybanhang.sanpham_copy AS S
    WHERE C.SOHD = NEW.SOHD AND C.MASP = S.MASP;
	IF (CTHDON * S.GIA) = NEW.TRIGIA THEN
        SIGNAL SQLSTATE '45000' 
		SET MESSAGE_TEXT = 'THANH CONG';
	ELSE
		SIGNAL SQLSTATE '45000' 
		SET MESSAGE_TEXT = 'KHONG THE CHEN VI TRI GIA LA SL * DON HANG';
    END IF;
END //
...
-- 11
DELIMITER //
CREATE TRIGGER CK_DOANHSO
BEFORE INSERT ON th1quanlybanhang.KHACHHANG_COPY
FOR EACH ROW 
BEGIN
	DECLARE TRIGIA INT;
    SELECT H.TRIGIA INTO TRIGIA
    FROM th1quanlybanhang.HOADON AS H
    WHERE H.MAKH = NEW.MAKH;
	IF SUM(TRIGIA) = NEW.DOANHSO THEN
        SIGNAL SQLSTATE '45000' 
		SET MESSAGE_TEXT = 'THANH CONG';
	ELSE
		SIGNAL SQLSTATE '45000' 
		SET MESSAGE_TEXT = 'KHONG THE CHEN VI DOANH SO LA TONG TRI GIA CUA CAC DON HANG';
    END IF;
END //
